...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(0, args.epochs)):
        ...
        if epoch % 5 == 0:
            ...
            print(
                f'acc_test: {return_dict["accuracy_score_test"]:.4f}, macro_f1_test: {return_dict["macro_f1_test"]:.4f}, rank: {return_dict["rank"]:.4f}'
            )
    ...
    if args.write == 1:
        if args.cluster == 1:
            ...
            #     line = f"gcn,{args.dataset},{args.rank:.1f},{args.embedding_dim:.1f},{args.p:.1f},{args.q:.1f},{args.feature_dim:.8f},{args.seed:.1f}," + comp_string + "\n"
            #     file.write(line)

            # np.save(f"../../1_classification/gcn_{args.dataset}_{args.rank:.0f}_{args.embedding_dim:.0f}_{args.p:.0f}_{args.q:.0f}_{args.feature_dim:.8f}_{args.seed:.0f}.npy", prediction_dict)
            ...
            with open(f"../example_results/2_node_classification_completed.txt", "a") as file:
                line = (
                    f"mlp,{args.dataset},{args.rank:.1f},{args.embedding_dim:.1f},{args.p:.1f},{args.q:.1f},{args.feature_dim:.8f},{args.seed:.1f},"
                    + comp_string
                    + "\n"
                )
                file.write(line)
            ...
            np.save(
                f"../example_results/mlp_nc_{args.dataset}_{args.rank:.0f}_{args.embedding_dim:.0f}_{args.p:.0f}_{args.q:.0f}_{args.feature_dim:.8f}_{args.seed:.0f}.npy",
                prediction_dict,
            )
