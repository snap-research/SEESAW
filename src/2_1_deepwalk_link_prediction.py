...
parser.add_argument(
    "--epochs", type=int, default=400, help="Number of epochs to train."  # 100
)
...
parser.add_argument(
    "--epoch_threshold", type=int, default=300, help="Dimension of embeddings."
)
...
if __name__ == "__main__":
    ...
    def dealing_with_edges(data):
        weights = np.ones(data.edge_index.shape[1])
    ...
    def train_an_mlp(
        ...
        predictor = MlpProdDecoder(args.embedding_dim, hidden_size=256).to(device)
    ...
    if args.write == 1:
        if args.cluster == 1:
            ...
            #         f.write("model,dataset,roc_score_val,ap_score_val,roc_score_test,ap_score_test,rank,rank_bound,embedding_dim,p,q,feature_dim,seed," + comp_string_keys + ",compidx\n")

            # with open(f"../../1_results/1_link_pred_completed.txt", "a") as file:
            #     line = f"deepwalk,{args.dataset},{roc_score_val:.4f},{ap_score_val:.4f},{roc_score_test:.4f},{ap_score_test:.4f},{rank:.1f},{args.rank:.1f},{args.embedding_dim:.1f},{args.p:.1f},{args.q:.1f},{args.feature_dim:.8f},{args.seed:.1f}," + comp_string + "," + str(args.comp_idx) + "\n"
            #     file.write(line)

            # np.save(f"../../1_link_prediction/deepwalk_{args.dataset}_{args.rank:.0f}_{args.embedding_dim:.0f}_{args.p:.0f}_{args.q:.0f}_{args.feature_dim:.8f}_{args.seed:.0f}.npy", prediction_dict)

        elif args.cluster == 0:
            if not os.path.exists("../example_results/2_link_pred_completed.txt"):
                print(f"2_link_pred_completed.txt does not exist, creating it...")

                with open("../example_results/2_link_pred_completed.txt", "a") as f:
                    formatted_comp_keys = list(hits_and_degree_bias.keys())
                    comp_string_keys = ",".join(formatted_comp_keys)
                    f.write(
            ...
            with open(f"../example_results/2_link_pred_completed.txt", "a") as file:
                line = (
                    f"deepwalk,{args.dataset},{roc_score_val:.4f},{ap_score_val:.4f},{roc_score_test:.4f},{ap_score_test:.4f},{rank:.1f},{args.rank:.1f},{args.embedding_dim:.1f},{args.p:.1f},{args.q:.1f},{args.feature_dim:.8f},{args.seed:.1f},"
                    + comp_string
                    + ","
                    + str(args.comp_idx)
                    + "\n"
            ...
            np.save(
                f"../example_results/deepwalk_2_lp_{args.dataset}_{args.rank:.0f}_{args.embedding_dim:.0f}_{args.p:.0f}_{args.q:.0f}_{args.feature_dim:.8f}_{args.seed:.0f}.npy",
                prediction_dict,
            )
