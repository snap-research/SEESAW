...
if __name__ == "__main__":
    ...
    if args.write == 1:
        if args.cluster == 1:
            ...
            #     line = f"gcn,{args.dataset},{roc_score_val:.4f},{ap_score_val:.4f},{roc_score_test:.4f},{ap_score_test:.4f},{rank:.1f},{args.rank:.1f},{args.embedding_dim:.1f},{args.p:.1f},{args.q:.1f},{args.feature_dim:.8f},{args.seed:.1f}," + comp_string + "," + str(args.comp_idx) + "\n"
            #     file.write(line)

            # np.save(f"../../1_link_prediction/gcn_{args.dataset}_{args.rank:.0f}_{args.embedding_dim:.0f}_{args.p:.0f}_{args.q:.0f}_{args.feature_dim:.8f}_{args.seed:.0f}.npy", prediction_dict)
            ...
            with open(f"../example_results/2_link_pred_completed.txt", "a") as file:
                line = (
                    f"mlp,{args.dataset},{roc_score_val:.4f},{ap_score_val:.4f},{roc_score_test:.4f},{ap_score_test:.4f},{rank:.1f},{args.rank:.1f},{args.embedding_dim:.1f},{args.p:.1f},{args.q:.1f},{args.feature_dim:.8f},{args.seed:.1f},"
                    + comp_string
                    + ","
                    + str(args.comp_idx)
                    + "\n"
            ...
            np.save(
                f"../example_results/mlp_lp_{args.dataset}_{args.rank:.0f}_{args.embedding_dim:.0f}_{args.p:.0f}_{args.q:.0f}_{args.feature_dim:.8f}_{args.seed:.0f}.npy",
                prediction_dict,
            )
